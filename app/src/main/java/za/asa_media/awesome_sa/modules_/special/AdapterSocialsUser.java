...
public class AdapterSocialsUser extends PagerAdapter {
    Activity context;
    String netImages[];
    LayoutInflater layoutInflater;
    int identityCode = 0;

    public AdapterSocialsUser(Activity context, String netImages[]) {
    ...
    public void setImage(String image_path, ImageView mBatchImage) {
        if (!TextUtils.isEmpty(image_path)) {
            Picasso.with(context).load(image_path).error(R.drawable.special_banner)
                    .placeholder(R.drawable.special_banner)
                    .skipMemoryCache()
                    .into(mBatchImage);
