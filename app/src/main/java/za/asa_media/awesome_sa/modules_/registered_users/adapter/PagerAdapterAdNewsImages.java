...
public class PagerAdapterAdNewsImages extends PagerAdapter {
    ...
    public PagerAdapterAdNewsImages(Activity context, String netImages[], int images[], int identityCode) {
        ...
        if (identityCode == 1) {
            if (context instanceof CreateAdNews) {
                mCallback = (IAdsImageCallback) context;
            }
        } else if (identityCode == 2) {
            if (context instanceof EditAdNews) {
                mCallback = (IAdsImageCallback) context;
            }
        }
    ...
    public void setImage(String image_path, ImageView mBatchImage) {
        if (!TextUtils.isEmpty(image_path)) {
            Picasso.with(context).load(image_path).error(R.drawable.img_news)
